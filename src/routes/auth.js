...
router.post('/register', async (req, res) => {
  ...
  try {
    ...
    // Generate a unique roll_number based on email (first part before @)
    const baseRollNumber = email.split('@')[0].substring(0, 10).toUpperCase();
    let rollNumber = baseRollNumber;
    let counter = 1;
    
    // Check if roll_number already exists and generate a unique one
    let rollExists = true;
    while (rollExists) {
      const [existing] = await db.query('SELECT id FROM users WHERE roll_number = ?', [rollNumber]);
      if (existing.length === 0) {
        rollExists = false;
      } else {
        rollNumber = `${baseRollNumber}${counter}`;
        counter++;
      }
    }
    ...
    const [result] = await db.query(
      'INSERT INTO users (roll_number, name, email, phone, password, role, branch, year, profile_completed) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
      [rollNumber, name, email, phone, hashedPassword, role || 'student', branch || null, year || null, 0]
    );
...
router.post('/login', async (req, res) => {
  ...
  try {
    ...
    const looksHashed = typeof storedPassword === 'string' && /^\$2[aby]\$\d{2}\$/.test(storedPassword);

    let match = false;
    if (looksHashed) {
      match = await bcrypt.compare(password, storedPassword);
    } else {
      // Backward compatibility for users inserted directly into DB with plain-text passwords.
      match = storedPassword === password;
      if (match) {
        // Migrate legacy password to bcrypt hash immediately after successful login.
        const hashedPassword = await bcrypt.hash(password, 10);
...
router.get('/users/:userId/clubs', async (req, res) => {
  ...
  try {
    const [clubs] = await db.query(
      `SELECT c.id, c.name, c.description, c.admin_id, cm.role, cm.joined_at
       FROM clubs c

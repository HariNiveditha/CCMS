...
router.get('/admin/:adminId', async (req, res) => {
  ...
  try {
    const [clubs] = await db.query(
      'SELECT id, name, description, admin_id, recruitmentOpen, eventRegistrationsOpen, created_at FROM clubs WHERE admin_id = ? ORDER BY created_at DESC',
      [adminId]
    );
    res.json({ success: true, data: clubs });
  } catch (err) {
...
router.post('/:clubId/request-join', async (req, res) => {
  ...
  try {
    const [clubRows] = await db.query('SELECT * FROM clubs WHERE id = ?', [clubId]);

    if (clubRows.length === 0) {
      return res.status(404).json({ success: false, message: 'Club not found' });
    }

    if (!isToggleEnabled(clubRows[0], 'recruitmentOpen', 'recruitment_open')) {
      return res.status(403).json({ success: false, message: 'Recruitment is currently closed for this club' });
    }
    ...
    if (year || branch || rollNumber) {
      ...
      if (year && userColumnSet.has('year')) {
        updates.push('year = ?');
        values.push(year);
      }
      if (branch && userColumnSet.has('branch')) {
        updates.push('branch = ?');
        values.push(branch);
      }
      if (rollNumber && userColumnSet.has('roll_number')) {
...
// PUT toggle event registrations status (admin only)
router.put('/:clubId/event-registrations', async (req, res) => {
  try {
    const clubId = req.params.clubId;
    const { eventRegistrationsOpen } = req.body;

    if (eventRegistrationsOpen === undefined) {
      return res.status(400).json({ success: false, message: 'eventRegistrationsOpen is required' });
    }

    const [result] = await db.query(
      'UPDATE clubs SET eventRegistrationsOpen = ? WHERE id = ?',
      [eventRegistrationsOpen ? 1 : 0, clubId]
    );
    ...
    res.json({
      success: true,
      message: eventRegistrationsOpen ? '✅ Event registrations opened!' : '✅ Event registrations closed!'
    });
  } catch (err) {
    res.status(500).json({ success: false, message: 'Server error', error: err.message });
  }

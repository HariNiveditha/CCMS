...
async function initializeApp() {
  ...
  // 3. Load member data
  await loadMemberData(memberId, clubId);
...
//  DATA FETCHING
// ════════════════════════════════════════════════════════════════
async function loadMemberData(memberId, clubId = null) {
  try {
    showLoading(true);
    ...
    if (clubId) {
      ...
      if (!clubResponse.ok) {
        throw new Error(`Failed to load club data (${clubResponse.status})`);
      }

      const clubResult = await clubResponse.json();
      const membersResult = await membersResponse.json();

      if (!clubResult.success || !membersResult.success) {
        throw new Error('Failed to load member data');
      }

      const member = Array.isArray(membersResult.data)
        ? membersResult.data.find(item => String(item.user_id) === String(memberId))
        : null;
      ...
      currentClubName = clubResult.data.name || null;
      result = { success: true, data: { ...member, club_name: currentClubName } };
    } else {
...
function displayMemberProfile() {
  ...
  // Dates
  const joinDate = joined_at || joined_date || created_at;
